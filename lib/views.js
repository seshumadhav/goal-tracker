...
const BASE_STYLE = `
  ...
  * { box-sizing: border-box; }
  ...
  .today-list, .progress-list { list-style: none; margin: 1rem 0 0; padding: 0; display: flex; flex-direction: column; gap: 0.6rem; }
  .today-row, .progress-row {
    display: flex;
    align-items: center;
    justify-content: space-between;
    gap: 0.75rem;
  ...
  .row-name { font-weight: 600; text-decoration: none; color: var(--fg); }
  .row-stat { font-size: 0.88rem; color: var(--muted); white-space: nowrap; }
  .row-stat.overdue { color: var(--danger); font-weight: 600; }
  .today-form { display: flex; align-items: center; gap: 0.5rem; flex: none; }
  .today-form input {
    width: 6.5rem;
  ...
  .today-actions { display: flex; gap: 0.4rem; }
  .icon-btn.active-success { background: var(--accent); color: var(--accent-fg); border-color: var(--accent); }
  .icon-btn.active-fail { background: var(--danger); color: var(--bg); border-color: var(--danger); }
  .icon-btn.small { width: 2rem; height: 2rem; }
  .icon-btn.small svg { width: 15px; height: 15px; }
  .streak-count { display: inline-flex; align-items: center; gap: 0.3rem; font-weight: 600; color: var(--fg); }
  .streak-count svg { width: 15px; height: 15px; color: var(--accent); }
`;
...
const ICON_FLAME =
  '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2c1 3-3 4-3 8a3 3 0 0 0 6 0c1.5 1 2 3 2 4.5A5.5 5.5 0 0 1 6.5 20 5.5 5.5 0 0 1 3 15c0-4 3-6 4-9 .3 1.5 1 2 1 2s.5-3-1-6c3 0 5 2 5 5Z"/></svg>';
const ICON_LIST =
...
function topbar({ backHref, right } = {}) {
  const back = backHref
    ? `<a class="icon-btn" href="${escapeHtml(backHref)}" aria-label="Back">${ICON_BACK}</a>`
    : '';
  return `<header class="topbar" style="justify-content:space-between">
    <div style="display:flex;align-items:center;gap:0.75rem">${back}<a class="brand" href="/">GOAT</a></div>
    ${right || ''}
  </header>`;
}
...
function todayRow(goal, entriesForGoal, today) {
  const todayEntry = entriesForGoal.find((e) => e.date === today);
  const name = `<a class="row-name" href="/goals/${escapeHtml(goal.id)}">${escapeHtml(goal.name)}</a>`;

  if (goal.type === 'target') {
    const val = todayEntry && todayEntry.value !== null ? todayEntry.value : '';
    return `<li class="today-row">
      ${name}
      <form class="today-form" method="POST" action="/goals/${escapeHtml(goal.id)}/log">
        <input type="number" step="any" name="value" placeholder="${escapeHtml(goal.unit || 'value')}" value="${escapeHtml(
      val
    )}" required />
        <button class="icon-btn small" type="submit" aria-label="Log ${escapeHtml(goal.name)}">${ICON_CHECK}<span class="sr-only">Log</span></button>
      </form>
    </li>`;
  }

  if (goal.type === 'aggregate') {
    const loggedToday = todayEntry ? todayEntry.value : null;
    const placeholder = loggedToday ? `+${loggedToday}` : goal.unit || 'value';
    return `<li class="today-row">
      ${name}
      <form class="today-form" method="POST" action="/goals/${escapeHtml(goal.id)}/log">
        <input type="number" step="any" name="value" placeholder="${escapeHtml(placeholder)}" required />
        <button class="icon-btn small" type="submit" aria-label="Add to ${escapeHtml(goal.name)}">${ICON_CHECK}<span class="sr-only">Add</span></button>
      </form>
    </li>`;
  }

  const successState = todayEntry ? todayEntry.success : null;
  return `<li class="today-row">
    ${name}
    <div class="today-actions">
      <form method="POST" action="/goals/${escapeHtml(goal.id)}/log">
        <input type="hidden" name="success" value="true" />
        <button class="icon-btn small${successState === true ? ' active-success' : ''}" type="submit" aria-label="Mark ${escapeHtml(
    goal.name
  )} done today">${ICON_CHECK}<span class="sr-only">Success today</span></button>
      </form>
      <form method="POST" action="/goals/${escapeHtml(goal.id)}/log">
        <input type="hidden" name="success" value="false" />
        <button class="icon-btn small danger${successState === false ? ' active-fail' : ''}" type="submit" aria-label="Mark ${escapeHtml(
    goal.name
  )} failed today">${ICON_X}<span class="sr-only">Failed today</span></button>
      </form>
    </div>
  </li>`;
}

function progressRow(goal, entriesForGoal, today) {
  const name = `<a class="row-name" href="/goals/${escapeHtml(goal.id)}">${escapeHtml(goal.name)}</a>`;

  if (goal.type === 'target') {
    const p = progress.targetProgress(goal, entriesForGoal, today);
    const stat = p.overdue
      ? 'Overdue'
      : `${p.percent}%${p.daysRemaining !== null ? ` · ${p.daysRemaining}d left` : ''}`;
    return `<li class="progress-row">${name}<span class="row-stat${p.overdue ? ' overdue' : ''}">${escapeHtml(
      stat
    )}</span></li>`;
  }

  if (goal.type === 'aggregate') {
    const p = progress.aggregateProgress(goal, entriesForGoal, today);
    const stat = p.overdue ? 'Overdue' : `${p.total}/${goal.targetTotal} ${goal.unit || ''}`.trim();
    return `<li class="progress-row">${name}<span class="row-stat${p.overdue ? ' overdue' : ''}">${escapeHtml(
      stat
    )}</span></li>`;
  }

  const length = progress.streakLength(entriesForGoal, today);
  return `<li class="progress-row">${name}<span class="streak-count">${ICON_FLAME}${length}</span></li>`;
}

function homePage(goals, entriesByGoal, today) {
  if (goals.length === 0) {
    return page(
      'GOAT',
      `
      ...
      <main>
        <div class="hero">
          <p class="eyebrow">Welcome</p>
          <h1>GOAT</h1>
          <p>No goals yet — add your first one to start tracking.</p>
          <a class="icon-btn" style="width:2.75rem;height:2.75rem;border-color:var(--accent);color:var(--accent)" href="/goals/new" aria-label="Add goal">
            ${ICON_PLUS}
          </a>
        </div>
  ...
  const todayItems = goals.map((g) => todayRow(g, entriesByGoal[g.id] || [], today)).join('');
  const progressItems = goals.map((g) => progressRow(g, entriesByGoal[g.id] || [], today)).join('');
  const goalsLink = `<a class="icon-btn" href="/goals" aria-label="Your goals">${ICON_LIST}</a>`;

  return page(
    'GOAT',
    `
    ${topbar({ right: goalsLink })}
    <main>
      <section>
        <p class="eyebrow">Today</p>
        <h1>Log today</h1>
        <ul class="today-list">${todayItems}</ul>
      </section>
      <section>
